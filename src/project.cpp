...
//-------------------------------------------------------------------//
// Utility function abs
//-------------------------------------------------------------------//
gdouble abs(gdouble _x)
{
  return _x<0?-_x:_x; 
}
...
point find_point_in_garray( GArray* _g, gdouble _x, gdouble _sensitivity )
{
  ...
  for( _i=0; _i<=_g->len; _i++ )
  {
    if( abs( g_array_index( _g, point, _i ).x - _x) < _sensitivity ) return g_array_index( _g, point, _i );
  }
//  return NULL;
}
...
gdouble check_dta(  GArray* _second, point* _p )
{
  ...
  /**/  g_string_append_printf( _s, "DIFF : is: %2.2f%%, should be lt: %2.1f%% ", abs( _first_dose - _second_dose ), _difference, NULL);
  if( abs( _first_dose - _second_dose ) < _difference )
  ...
  for(;;)
  {
    ...
    _diff_0 = abs( _first_dose - _second_dose );
  /**/  g_string_append_printf( _s, "D: %2.2f, DD: %2.2f ", _second_dose, _diff_0, NULL);
    ...
    _diff_p1 = abs( _first_dose - _second_dose );
    _second_dose = find_point_in_garray( _second, _x2-get_step_of_garray( _second ), 0.5*get_step_of_garray( _second ) ).dose;
    _diff_m1 = abs( _first_dose - _second_dose );
...
void compare_button_clicked(  )
{
  ...
  if( abs((get_step_of_garray(monaco_garray) / get_step_of_garray(omnipro_points_garray)) - 1.0) >= 0.001 )
...
{
  if( abs( _d1 - _d2 ) < _sensitivity ) return TRUE;
  return FALSE;
}
